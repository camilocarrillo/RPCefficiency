...
#include "STASkim/ProducerTest/plugins/MuonTimingValidator.h"
...
MuonTimingValidator::MuonTimingValidator(const edm::ParameterSet& iConfig):
  ...
  MuonTokens_(consumes<reco::TrackCollection>(MuonTags_)),
...
//  std::cout << " Event: " << iEvent.id() << "  Orbit: " << iEvent.orbitNumber() << "  BX: " << iEvent.bunchCrossing() << std::endl;
  ...
  const reco::TrackCollection muonC = *(MuCollection.product());
  ...
  reco::TrackCollection::const_iterator imuon;
  ...
  for(imuon = muonC.begin(); imuon != muonC.end(); ++imuon){
    ...
    reco::TrackRef trkTrack(MuCollection,i);
...
//     }
    ...
    reco::MuonTimeExtra timec = timeMapCmb[trkTrack];
    reco::MuonTimeExtra timedt = timeMapDT[trkTrack];
    reco::MuonTimeExtra timecsc = timeMapCSC[trkTrack];
